...
router.put('/order/:id', async (req, res) => {
    ...
    try {
            await dbConnection.query(`UPDATE orders SET status = ?,  description = ?, payment_method = ?, address = ?, product_id = ? WHERE id = ?;`, { replacements: [status, description, payment_method, address, product_id] });
